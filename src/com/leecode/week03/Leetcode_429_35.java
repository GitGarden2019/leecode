...
public class Leetcode_429_35 {
    ...
    /**
     * 思考过程，按照层次遍历只是可以打印顺序，一开始是直接输出 但是碰到有个问题的就是层次没注意， 第一把提交未通过，再寻找原因 后面发现每次入队列的时候队列上面的元素其实就是当前层的顺序，但是这次需要每次取元素的时候把
     * 队列里面的元素都拿出来 拿出来之后队列必然是空的，接下来就把下一层元素全部放进队列里面 每个元素会被访问到两次，入队列一次，出队列一次
     */
